...
- **Credits (2026-09-18)**: `clips.credits_for()` → `video_credits: [str]` stored on translator messages (`Message.video_credits`) and chat messages (set in `/chat/messages/{mid}/sign-video`). `BubbleVideo` prop `credits` renders "🎥 <source>" under the video (`<testID>-credits`); translator top panel shows `avatar-video-credits`. Cache key `v3` now includes the clip URL per token so switching sources never serves a stale video.
